...
export const useYhdm = (animeDetail: Bangumi.AnimeDeatilItem | null) => {
  const infoboxVal: Ref<Array<Bangumi.InfoBoxItem>> | undefined = inject('infoboxVal');
  // 找出谁是导演
  const eps = computed(() => {
  ...
  const getYhdmList = async (keywords: string) => {
    if (animeDetail && animeDetail.type == 2) {
      ...
      if (keywords.indexOf(' ') != -1) {
        ...
        const regEngname = new RegExp(/[a-zA-Z0-9]/);
  ...
  if (animeDetail) {
    sites = getSiteFromBGMId(animeDetail?.id, animeDetail?.air_date);
    const keywords = animeDetail.name_cn ? animeDetail.name_cn : animeDetail.name;
